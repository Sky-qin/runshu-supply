...
export default {
  namespace: "inventoryProfitModel",
  state: {
  ...
  effects: {
    *getTableList({ payload }, { call, put, select }) {
      const { pagination, searchParams } = yield select(
        (state) => state.inventoryProfitModel
      );
      ...
      const { data } = yield call(API.inventoryProfitList, params);
    ...
    *findTabulatorList({ payload }, { call, put, select }) {
      const { data } = yield call(API.findTabulatorList, { type: 1 });
      if (data && data.success) {
        yield put({
          type: "save",
          payload: {
    ...
    *sendCheckInfo({ payload }, { call, put, select }) {
      ...
      yield put({ type: "save", payload: { drawerLoading: true } });
      const { data } = yield call(API.addSurplusOrder, {
        stockId,
        remarks,
